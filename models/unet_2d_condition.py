...
class UNet2DConditionModel(ModelMixin, ConfigMixin, UNet2DConditionLoadersMixin):
    ...
    @classmethod
    def from_pretrained_orig(cls, pretrained_model_path, daspsr_model_path, subfolder=None, use_image_cross_attention=False, **kwargs):
        if subfolder is not None:
            pretrained_model_path = os.path.join(pretrained_model_path, subfolder)
            daspsr_model_path = os.path.join(daspsr_model_path, subfolder)
        ...
        model_file_daspsr = os.path.join(daspsr_model_path, SAFETENSORS_WEIGHTS_NAME)
        if not os.path.isfile(model_file):
            raise RuntimeError(f"{model_file} does not exist")
        if not os.path.isfile(model_file_daspsr):
            raise RuntimeError(f"{model_file_daspsr} does not exist")
        state_dict = safetensors.torch.load_file(model_file, device="cpu")
        state_dict_daspsr = safetensors.torch.load_file(model_file_daspsr, device="cpu")
        for k, v in state_dict_daspsr.items():
